...
def get_github_docs(url):
    return process_github_repo(url)
...
@st.cache_resource
def build_index(repo_url: str, _docs):
    # unique index name per repo URL
    h = hashlib.md5(repo_url.encode()).hexdigest()[:8]
    index_name = f"repo-pdf-rag-{h}"
    return setup_rag_chain(_docs, index_name=index_name)
    ...
    if not repo_url:
        ...
        try:
            ...
            with st.spinner("Building/Retrieving index…"):
                vs = build_index(repo_url, gh_docs + pdf_docs)
